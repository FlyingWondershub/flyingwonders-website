...
async function run() {
  ...
  const missing = docs.filter(d => d.name.includes('Harry Potter') || d.name.includes('Fun Pass'));

  for (const doc of missing) {
    console.log(`Uploading fallback image for: ${doc.name}`);
    const res = await fetch('https://images.unsplash.com/photo-1502680390469-be75c86b636f?auto=format&fit=crop&w=800&q=80');
    if (!res.ok) continue;
