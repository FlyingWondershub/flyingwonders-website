...
function getAttractionVehicle(
  vList: any[],
  vIdx?: number,
  vType?: string
): { type: string; pricePerTransfer: number; serviceName?: string; compositeKey?: string; vehicleType?: string } {
  if (!Array.isArray(vList) || vList.length === 0) {
    return { type: '13-Seater - Private', pricePerTransfer: 45, serviceName: 'Transfers', compositeKey: '13-Seater - Private - group - Transfers' }
  }

  // 1. If explicit vehicleType is provided, prioritize matching by type and service
  if (vType) {
    const cleanType = vType.toLowerCase().trim()
    const found = vList.find(v => 
      v.type?.toLowerCase().trim() === cleanType && 
      (v.serviceName?.toLowerCase().includes('transfer') || v.type?.toLowerCase().includes('transfer'))
    ) || vList.find(v => v.type?.toLowerCase().trim() === cleanType)
    if (found) return found
  }

  // 2. If valid numeric index is provided and points to a transfer service
  if (typeof vIdx === 'number' && vIdx >= 0 && vList[vIdx]) {
    const v = vList[vIdx]
    const sName = (v.serviceName || '').toLowerCase()
    // Valid if explicitly a transfer or not a conflicting service like disposal or arrivals
    if (sName.includes('transfer') || (!sName.includes('disposal') && !sName.includes('arrival') && !sName.includes('departure') && !sName.includes('city tour') && !sName.includes('pickup'))) {
      return v
    }
  }

  // 3. Fallback to standard 13-Seater transfer
  const def13Idx = get13SeaterVehicleIndex(vList, 'transfer')
  return vList[def13Idx] || vList[0]
}
...
export default function PrototypeBuilder() {
  ...
  // Dedicated Land Package Quoter States
  const [landPackageModalItem, setLandPackageModalItem] = useState<any | null>(null)
  const [landPackageDate, setLandPackageDate] = useState('')
  const [landPackageAdults, setLandPackageAdults] = useState(2)
  const [landPackageKids, setLandPackageKids] = useState(0)
  const [landPackageChildAges, setLandPackageChildAges] = useState<number[]>([])
  const [landPackageTransferMode, setLandPackageTransferMode] = useState<'sic' | 'private13'>('private13')
  const [landPackageDays, setLandPackageDays] = useState<any[]>([])
  const [landPackageMarkupPercent, setLandPackageMarkupPercent] = useState(0)
  const [landPackageGuestName, setLandPackageGuestName] = useState('')
  const [landPackageGuestPhone, setLandPackageGuestPhone] = useState('')
  const [landPackageAttractionModalDay, setLandPackageAttractionModalDay] = useState<number | null>(null)
  const [landPackageSearchAttraction, setLandPackageSearchAttraction] = useState('')
  const [landPackageNewTransferDay, setLandPackageNewTransferDay] = useState<number | null>(null)
  ...
  useEffect(() => {
    ...
    fetch('/api/site-settings')
      .then(res => res.json())
      .then(data => {
        if (data.settings?.defaultLandPackageTerms) setDefaultLandPackageTerms(data.settings.defaultLandPackageTerms)
    ...
    fetch('/api/ready-packages')
      ...
      .then(data => {
        if (data.success && Array.isArray(data.templates) && data.templates.length > 0) {
          setReadyTemplatesList(data.templates)
        }
      })
      .catch(() => {})
      ...
      return prevItin.map(day => ({
        ...
        attractions: (day.attractions || []).map(a => {
          if (attractionsList.length > 0 && a.attractionName) {
            ...
            if (vehiclesList.length > 0 && a.hasTransfer) {
              const pv = getAttractionVehicle(vehiclesList, a.pickupVehicleIndex, a.pickupVehicleType)
              const pvIdx = vehiclesList.indexOf(pv)
              if (pvIdx >= 0) updated = { ...updated, pickupVehicleIndex: pvIdx, pickupVehicleType: pv.type }

              const dv = getAttractionVehicle(vehiclesList, a.dropVehicleIndex, a.dropVehicleType)
              const dvIdx = vehiclesList.indexOf(dv)
              if (dvIdx >= 0) updated = { ...updated, dropVehicleIndex: dvIdx, dropVehicleType: dv.type }
            }
            return updated
          }
          return a
  ...
  const openLandPackageQuoter = (tmpl: any) => {
    if (!tmpl) return
    setLandPackageModalItem(tmpl)
    setLandPackageDate(arrivalDate || minCheckinDate)
    setLandPackageAdults(adults > 0 && adults <= 12 ? adults : 2)
    setLandPackageKids(kids >= 0 && kids <= 12 ? kids : 0)
    setLandPackageChildAges(childAges.length > 0 ? [...childAges] : [])
    setLandPackageTransferMode('private13')
    setLandPackageMarkupPercent(markupPercent || 0)
    setLandPackageGuestName(guestName || '')
    setLandPackageGuestPhone(guestPhone || '')
    setLandPackageSavedProposalNum(null)
    setLandPackageSavingProposal(false)
    setLandPackageNewTransferHours(4)

    if (tmpl.itinerary && Array.isArray(tmpl.itinerary)) {
      const clonedDays = tmpl.itinerary.map((d: any, idx: number) => ({
        ...
        attractions: Array.isArray(d.attractions) ? d.attractions.map((a: any) => {
          ...
          return {
            ...
            time: a.time || '14:00',
            inclusionsNotes: a.inclusionsNotes || a.notes || 'Admission Ticket',
            isOptional: !!a.isOptional
          }
        }) : []
      }))
  ...
  // Live Price Calculation for Land Package (Google Sheet pricing)
  const landPackageCalculation = useMemo(() => {
    if (!landPackageModalItem) return null

    const totalPax = landPackageAdults + landPackageKids
    const isOverCapacity = totalPax > 12

    // 1. Google Sheet Transfer Rates
    // 13-Seater Minibus rates:
    const v13Arrival = vehiclesList.find(v => (v.vehicleType?.toLowerCase().includes('13') || v.type?.toLowerCase().includes('13')) && (v.serviceName?.toLowerCase().includes('arrival') || v.type?.toLowerCase().includes('arrival')))
    const v13Departure = vehiclesList.find(v => (v.vehicleType?.toLowerCase().includes('13') || v.type?.toLowerCase().includes('13')) && (v.serviceName?.toLowerCase().includes('departure') || v.type?.toLowerCase().includes('departure')))
    const v13CityTour = vehiclesList.find(v => (v.vehicleType?.toLowerCase().includes('13') || v.type?.toLowerCase().includes('13')) && (v.serviceName?.toLowerCase().includes('city') || v.type?.toLowerCase().includes('city')))
    const v13Transfer = vehiclesList.find(v => (v.vehicleType?.toLowerCase().includes('13') || v.type?.toLowerCase().includes('13')) && (v.serviceName?.toLowerCase().includes('transfer') || v.type?.toLowerCase().includes('transfer')))
    ...
    const rate13Arrival = v13Arrival?.pricePerTransfer || 45
    const rate13Departure = v13Departure?.pricePerTransfer || 45
    const rate13CityTour = v13CityTour?.pricePerTransfer || 120
    const rate13Transfer = v13Transfer?.pricePerTransfer || 45
    const rate13Disposal = v13Disposal?.pricePerTransfer || 45

    // SIC rates:
    const sicCityTour = vehiclesList.find(v => isVehicleSIC(v) && (v.serviceName?.toLowerCase().includes('city') || v.type?.toLowerCase().includes('city')))
    const sicTransfer = vehiclesList.find(v => isVehicleSIC(v) && (v.serviceName?.toLowerCase().includes('transfer') || v.type?.toLowerCase().includes('transfer') || v.serviceName?.toLowerCase().includes('round')))

    const rateSicCityTour = sicCityTour?.pricePerTransfer || 15
    const rateSicTransfer = sicTransfer?.pricePerTransfer || 12
      ...
      (d.transfers || []).forEach((tr: any) => {
        ...
        if (sType === 'arrival') {
          ...
          if (landPackageTransferMode === 'sic') {
            sightseeingTransfersCost += rateSicCityTour * Math.max(1, totalPax)
          } else {
            sightseeingTransfersCost += rate13CityTour
          }
        } else {
          if (landPackageTransferMode === 'sic') {
            sightseeingTransfersCost += rateSicTransfer * Math.max(1, totalPax)
          } else {
            sightseeingTransfersCost += rate13Transfer
          }
        }
      })
    })
    ...
    // 2. Google Sheet Attraction Rates with Child Ages
    let adultTicketCount = landPackageAdults
    let childTicketCount = 0
    let infantCount = 0

    for (let i = 0; i < landPackageKids; i++) {
      const age = landPackageChildAges[i] ?? 5
      if (age <= 2) {
        infantCount++
      } else if (age >= 13) {
        adultTicketCount++
    ...
    const markupFactor = 1 + (landPackageMarkupPercent / 100)
  ...
  }, [landPackageModalItem, landPackageDays, landPackageAdults, landPackageKids, landPackageChildAges, landPackageTransferMode, landPackageMarkupPercent, vehiclesList, attractionsList, sgdToInrRate])

  const addLandPackageAttraction = (dayIdx: number, attr: any) => {
    setLandPackageDays(prev => {
      const copy = [...prev]
      if (copy[dayIdx]) {
        copy[dayIdx] = {
          ...copy[dayIdx],
    ...
    setLandPackageAttractionModalDay(null)
    setLandPackageSearchAttraction('')
    showToast(`Added ${attr.name} to Day ${dayIdx + 1}! 🎟️`, 'success')
  }

  const removeLandPackageAttraction = (dayIdx: number, aIdx: number) => {
    setLandPackageDays(prev => {
      const copy = [...prev]
      if (copy[dayIdx]) {
        copy[dayIdx] = {
          ...copy[dayIdx],
          attractions: (copy[dayIdx].attractions || []).filter((_: any, i: number) => i !== aIdx)
        }
      }
      return copy
    })
  ...
  const addLandPackageTransfer = (dayIdx: number) => {
    ...
    setLandPackageDays(prev => {
      ...
      if (copy[dayIdx]) {
        copy[dayIdx] = {
          ...copy[dayIdx],
          transfers: [
            ...(copy[dayIdx].transfers || []),
            {
              serviceType: newTransferServiceType,
              routeDescription: newTransferRoute.trim(),
              time: newTransferTime || '14:00',
              hours: newTransferServiceType === 'disposal' ? (landPackageNewTransferHours || 4) : undefined
            }
          ]
        }
      }
    ...
    setLandPackageNewTransferDay(null)
    setNewTransferRoute('')
    setLandPackageNewTransferHours(4)
    showToast(`Added transfer to Day ${dayIdx + 1}! 🚗`, 'success')
  }

  const updateLandPackageTransferHours = (dayIdx: number, tIdx: number, hours: number) => {
    const validHours = Math.max(1, Math.min(24, hours || 4))
    setLandPackageDays(prev => {
      const copy = [...prev]
      if (copy[dayIdx] && copy[dayIdx].transfers?.[tIdx]) {
        const trs = [...copy[dayIdx].transfers]
        trs[tIdx] = { ...trs[tIdx], hours: validHours }
        copy[dayIdx] = { ...copy[dayIdx], transfers: trs }
      }
      return copy
    })
  }

  const removeLandPackageTransfer = (dayIdx: number, tIdx: number) => {
    setLandPackageDays(prev => {
      const copy = [...prev]
      if (copy[dayIdx]) {
        copy[dayIdx] = {
          ...copy[dayIdx],
          transfers: (copy[dayIdx].transfers || []).filter((_: any, i: number) => i !== tIdx)
        }
      }
      return copy
    })
  ...
  const generateLandPackageWhatsAppText = () => {
    if (!landPackageModalItem || !landPackageCalculation) return ''
    const pNum = landPackageSavedProposalNum || `FW-LAND-${Math.floor(100000 + Math.random() * 900000)}`
    const sep = '━━━━━━━━━━━━━━━━━━━━━━━━━━'
    const title = landPackageModalItem?.title || 'Singapore Land Package'
    const dest = destinationMode === 'malaysia' ? 'MALAYSIA' : 'SINGAPORE'
    const childAgeStr = landPackageKids > 0 && landPackageChildAges.length > 0 ? ` (Ages: ${landPackageChildAges.slice(0, landPackageKids).join(', ')} yrs)` : ''
    const privateTransLabel = 'Private 13-Seater Minibus'
    const sicTransLabel = 'Shared Coach (SIC)'
    const modeLabel = landPackageTransferMode === 'sic' ? sicTransLabel : privateTransLabel

    let t = `✈️ *${dest} LAND PACKAGE ITINERARY*  (Ref: ${pNum})\n`
    t += `*${title}*\n`
    t += `${sep}\n`
    t += `👤 *Guest Name:* ${landPackageGuestName || 'Valued Guest'}\n`
    if (landPackageGuestPhone) t += `📞 *Guest Contact:* ${landPackageGuestPhone}\n`
    t += `👥 *Pax:* ${landPackageAdults} Adult${landPackageAdults !== 1 ? 's' : ''}${landPackageKids > 0 ? ` & ${landPackageKids} Child${landPackageKids !== 1 ? 'ren' : ''}${childAgeStr}` : ''}\n`
    t += `📅 *Travel Date:* ${landPackageDate || 'TBD'} (${landPackageModalItem?.nightsCount || 3}N/${(landPackageModalItem?.nightsCount || 3) + 1}D Land Package)\n`
    ...
    t += `💵 *Total Package:* S$ ${landPackageCalculation.totalClientPriceSGD.toLocaleString()}  _(≈ ₹${landPackageCalculation.totalClientPriceINR.toLocaleString('en-IN')})_\n`
    t += `👤 *Per Adult:* S$ ${landPackageCalculation.adultQuoteSGD.toLocaleString()}`
    if (landPackageCalculation.childTicketCount > 0) {
      t += `  |  👶 *Per Child:* S$ ${landPackageCalculation.childQuoteSGD.toLocaleString()}`
    }
    t += `\n${sep}\n\n`

    t += `📅 *DAY-BY-DAY ITINERARY:*\n`
    landPackageDays.forEach((day: any, idx: number) => {
      t += `\n*Day ${day.dayNumber || idx + 1}: ${day.dayTitle || 'Tour Day'}*\n`
      if (day.dayDescription) {
        t += `  _${day.dayDescription}_\n`
      }
      const dayItems: { time: string; text: string }[] = []
      ;(day.transfers || []).forEach((tr: any) => {
        const icon = tr.serviceType === 'arrival' ? '🛬' : tr.serviceType === 'departure' ? '🛫' : tr.serviceType === 'cityTour' ? '🏙️' : '🚗'
        ...
        if (tr.serviceType === 'arrival' || tr.serviceType === 'departure') {
          ...
          mStr = landPackageTransferMode === 'sic' ? 'Half-Day City Tour (Shared Coach / SIC)' : 'Half-Day City Tour (Private 13-Seater)'
        }
    ...
    t += `\n${sep}\n`
    t += `📌 *${landPackageModalItem?.termsAndInclusions || defaultLandPackageTerms}*\n`
    t += `${sep}\n`
    if (customAgencyName) {
      t += `📞 *Prepared by:* ${customAgencyName}\n`
      if (customAgencyPhone) t += `📱 ${customAgencyPhone} `
      if (customAgencyEmail) t += `✉️ ${customAgencyEmail}\n`
    }
  ...
  const handleCopyLandPackageWhatsApp = async (e?: React.MouseEvent) => {
  ...
  const handleSendLandPackageWhatsApp = (e?: React.MouseEvent) => {
    if (e) e.stopPropagation()
    try {
      const text = generateLandPackageWhatsAppText()
      if (!text) {
      ...
      const rawDigits = (landPackageGuestPhone || '').replace(/[^0-9]/g, '')
      const validPhone = rawDigits.length >= 10 ? rawDigits : ''
      let waText = text
      if (text.length > 1800) {
        const lines = text.split('\n')
        const header = lines.slice(0, 18).join('\n')
  ...
  // Handle Save Proposal Directly from Land Package Modal
  const handleSaveLandPackageProposal = async () => {
    if (!landPackageModalItem || !landPackageCalculation) return
    setLandPackageSavingProposal(true)
    try {
      const payload = {
        proposalNumber: landPackageSavedProposalNum || undefined,
        isTemplateBased: true,
        templateName: landPackageModalItem.title || 'Singapore Land Package',
        agentEmail: activeAgent?.email,
        agentId: (activeAgent as any)?._id,
        guestName: landPackageGuestName.trim() || 'Valued Guest',
        guestPhone: landPackageGuestPhone.trim() || '',
        adults: landPackageAdults,
        kids: landPackageKids,
        childAges: landPackageChildAges,
        arrivalDate: landPackageDate || new Date().toISOString().split('T')[0],
        nights: landPackageModalItem.nightsCount || 3,
        hotelRequired: false,
        transferMode: landPackageTransferMode,
        markupPercent: landPackageMarkupPercent,
        itineraryNotes: landPackageModalItem.termsAndInclusions || defaultLandPackageTerms,
        costBreakdown: {
          roomCostTotal: 0,
          suppCostTotal: 0,
          transportTotal: landPackageCalculation.totalTransfersNet,
          attractionTotal: landPackageCalculation.totalAttractionsNet,
          mealTotal: 0,
          guideTotal: 0,
          netCost: landPackageCalculation.totalNetCostSGD,
          totalClientPrice: landPackageCalculation.totalClientPriceSGD,
          totalClientPriceINR: landPackageCalculation.totalClientPriceINR,
          adultQuote: landPackageCalculation.adultQuoteSGD,
          childQuote: landPackageCalculation.childQuoteSGD
        },
        itinerary: landPackageDays
      }
  ...
  const handleDownloadLandPackagePDF = async () => {
    ...
    const privateTransLabel = 'Private 13-Seater Minibus'
    const sicTransLabel = 'Shared Coach (SIC)'
    const privateCityTourLabel = 'Half-Day City Tour (Private 13-Seater)'
    const sicCityTourLabel = 'Half-Day City Tour (Shared Coach / SIC)'
    ...
    const mappedItinerary: DayPlan[] = landPackageDays.map((d: any) => {
      ...
      const dayAttractions = (d.attractions || []).map((a: any) => {
        let aIdx = 0
        if (a.attractionName) {
    ...
    await downloadProposalPDF(false, customFooter, {
      ...
      overrideGuestName: landPackageGuestName || undefined,
      overrideGuestPhone: landPackageGuestPhone || undefined,
      overrideProposalNumber: landPackageSavedProposalNum || undefined
    })
  ...
  const updateAttractionVehicle = (dayIndex: number, rIdx: number, mode: 'pickup' | 'drop', vehicleIndex: number) => {
    const day = itinerary[dayIndex]
    if (!day) return
    const updated = [...day.attractions]
    const vObj = vehiclesList[vehicleIndex]
    if (mode === 'pickup') {
      updated[rIdx] = {
        ...updated[rIdx],
        pickupVehicleIndex: vehicleIndex,
        pickupVehicleType: vObj?.type || ''
      }
    } else {
      updated[rIdx] = {
        ...updated[rIdx],
        dropVehicleIndex: vehicleIndex,
        dropVehicleType: vObj?.type || ''
      }
    }
    updateDay(dayIndex, 'attractions', updated)
  }
      ...
      day.attractions.forEach(attrRow => {
        ...
        if (attrRow.hasTransfer) {
          if (attrRow.pickupEnabled !== false) {
            const pv = getAttractionVehicle(vehiclesList, attrRow.pickupVehicleIndex, attrRow.pickupVehicleType)
            if (pv) {
          ...
          if (attrRow.dropEnabled !== false) {
            const dv = getAttractionVehicle(vehiclesList, attrRow.dropVehicleIndex, attrRow.dropVehicleType)
            if (dv) {
      ...
      day.attractions.forEach(a => {
        ...
        if (a.hasTransfer) {
          if (a.pickupEnabled !== false) {
            totalTransfers++
            const pvName = getAttractionVehicle(vehiclesList, a.pickupVehicleIndex, a.pickupVehicleType)?.type
            if (pvName) usedVehicles.add(pvName.split(' - ')[0] || pvName)
          }
          if (a.dropEnabled !== false) {
            totalTransfers++
            const dvName = getAttractionVehicle(vehiclesList, a.dropVehicleIndex, a.dropVehicleType)?.type
    ...
    itinerary.forEach((day, dIdx) => {
      ...
      day.transfers.forEach(tr => {
        ...
        if (isCityTour) {
          ...
          transferText = isSic ? 'Half-Day City Tour (3 Hours) • Shared Coach (SIC)' : 'Half-Day City Tour (3 Hours) • Private 13-Seater Minibus'
        }
        dayItems.push({
          time: tr.time || '00:00',
      ...
      day.attractions.forEach(a => {
        ...
        if (a.hasTransfer && a.pickupEnabled !== false) {
          const pvName = getAttractionVehicle(vehiclesList, a.pickupVehicleIndex, a.pickupVehicleType)?.type || 'Transfer'
        ...
        if (a.hasTransfer && a.dropEnabled !== false) {
          const dvName = getAttractionVehicle(vehiclesList, a.dropVehicleIndex, a.dropVehicleType)?.type || 'Transfer'
...
💵 *Per Adult:* S$ ${costBreakdown.adultQuote}${kids > 0 ? ` | *Per Child:* S$ ${costBreakdown.childQuote}` : ''}`
  ...
  const generateEmailHtml = (overrideNum?: string, opts = copyOptions) => {
    ...
    let daysHtml = ''
    itinerary.forEach((day, dIdx) => {
      const rawItems: { time: string; html: string }[] = []
      day.transfers.forEach(tr => {
        const vObj = vehiclesList[tr.vehicleIndex]
        const v = vObj?.type || 'Transfer'
        const desc = tr.description || ''
        const isCityTour = tr.serviceType === 'cityTour' || desc.toLowerCase().includes('city tour')
        let labelText = `${v} ${desc ? `&bull; ${desc}` : ''}`
        if (isCityTour) {
          const isSic = desc.toLowerCase().includes('sic') || isVehicleSIC(vObj)
          labelText = isSic ? 'Half-Day City Tour (3 Hours) &bull; Shared Coach (SIC)' : 'Half-Day City Tour (3 Hours) &bull; Private 13-Seater Minibus'
        }
        rawItems.push({
          time: tr.time || '00:00',
          html: `<tr><td style="padding:6px 10px;border-bottom:1px solid #E2E8F0;font-size:13px;color:#64748B;width:75px;">${tr.time || '00:00'}</td><td style="padding:6px 10px;border-bottom:1px solid #E2E8F0;font-size:13px;color:#1E293B;">🚗 <strong>Transfer:</strong> ${labelText}</td></tr>`
        })
      })
      day.attractions.forEach(a => {
        const name = attractionsList[a.attractionIndex]?.name || 'Attraction'
        if (a.hasTransfer && a.pickupEnabled !== false) {
          ...
          rawItems.push({
            time: a.pickupTime || '09:00',
            html: `<tr><td style="padding:6px 10px;border-bottom:1px solid #E2E8F0;font-size:13px;color:#64748B;width:75px;">${a.pickupTime || '09:00'}</td><td style="padding:6px 10px;border-bottom:1px solid #E2E8F0;font-size:13px;color:#1E293B;">🚗 <strong>Pickup:</strong> For ${name} (${pvName})</td></tr>`
          })
        }
        rawItems.push({
          time: a.time || '10:00',
          html: `<tr><td style="padding:6px 10px;border-bottom:1px solid #E2E8F0;font-size:13px;color:#64748B;width:75px;">${a.time || '10:00'}</td><td style="padding:6px 10px;border-bottom:1px solid #E2E8F0;font-size:13px;color:#1E293B;">🎟️ <strong>Attraction:</strong> ${name} ${a.description ? `&bull; ${a.description}` : ''}</td></tr>`
        })
        if (a.hasTransfer && a.dropEnabled !== false) {
          const dvName = getAttractionVehicle(vehiclesList, a.dropVehicleIndex, a.dropVehicleType)?.type || 'Transfer'
          rawItems.push({
            time: a.dropTime || '17:00',
            html: `<tr><td style="padding:6px 10px;border-bottom:1px solid #E2E8F0;font-size:13px;color:#64748B;width:75px;">${a.dropTime || '17:00'}</td><td style="padding:6px 10px;border-bottom:1px solid #E2E8F0;font-size:13px;color:#1E293B;">🚗 <strong>Drop-off:</strong> Return from ${name} (${dvName})</td></tr>`
          })
        }
      })
      if (day.isBreakTrip) {
        rawItems.push({
          time: 'All Day',
          html: `<tr><td style="padding:6px 10px;border-bottom:1px solid #E2E8F0;font-size:13px;color:#64748B;width:75px;">All Day</td><td style="padding:6px 10px;border-bottom:1px solid #E2E8F0;font-size:13px;color:#1E293B;">🌴 <strong>Free & Easy:</strong> Leisure / Shopping Day</td></tr>`
        })
      }
      ...
      const items = rawItems.map(r => r.html)
  ...
  const downloadProposalPDF = async (
    ...
    landPackageOptions?: {
      ...
      overrideGuestName?: string
      overrideGuestPhone?: string
      overrideProposalNumber?: string
    }
  ) => {
    const isLandPkg = !!landPackageOptions?.isLandPackage
    const pNum = landPackageOptions?.overrideProposalNumber || (isLandPkg ? (landPackageSavedProposalNum || await ensureProposalSaved(true)) : await ensureProposalSaved(true))
    ...
    effectiveItinerary.forEach(d => {
      (d.transfers || []).forEach((t: any) => {
        ...
        if (isCityTour) {
          ...
          label = isSic
            ? 'Half-Day City Tour (3 Hours) • Shared Coach (SIC)'
            : 'Half-Day City Tour (3 Hours) • Private 13-Seater Minibus'
        } else if (isLandPkg) {
          label = desc || vehicle
        }
      ...
      ;(d.attractions || []).forEach(a => {
        if (a.hasTransfer) {
          if (a.pickupEnabled !== false) {
            const pvObj = getAttractionVehicle(vehiclesList, a.pickupVehicleIndex, a.pickupVehicleType)
            usedVehicleLookups.push({
          ...
          if (a.dropEnabled !== false) {
            const dvObj = getAttractionVehicle(vehiclesList, a.dropVehicleIndex, a.dropVehicleType)
            usedVehicleLookups.push({
    ...
    if (ctLine) doc.text(ctLine, agencyStartX, 44)
      ...
      const effGName = landPackageOptions?.overrideGuestName || guestName
      const effGPhone = landPackageOptions?.overrideGuestPhone || guestPhone
      const guestDisplay = `${effGName || 'Valued Guest'}${effGPhone ? ` (${effGPhone})` : ''}`
      ...
      effectiveItinerary.forEach(d => {
        ...
        d.attractions?.forEach(a => {
          if (!a.isOptional && a.hasTransfer) {
            if (a.pickupEnabled !== false) {
              const pv = getAttractionVehicle(vehiclesList, a.pickupVehicleIndex, a.pickupVehicleType)
              if (pv?.type) allVehiclesSet.add(pv.type)
            }
            if (a.dropEnabled !== false) {
              const dv = getAttractionVehicle(vehiclesList, a.dropVehicleIndex, a.dropVehicleType)
              if (dv?.type) allVehiclesSet.add(dv.type)
            }
          }
        })
      })
      ...
      // ─── CUSTOM ITINERARY NOTES & SPECIAL INSTRUCTIONS (Compact Card) ────
      if (itineraryNotes && itineraryNotes.trim()) {
        const noteParagraphs = itineraryNotes.trim().split('\n').filter(Boolean)
        font('normal', 6.8)
        let totalNoteLines = 0
        const splitParagraphs = noteParagraphs.map(np => {
          const clean = cleanPdfText(np.trim().replace(/^[•\-\*]\s*/, ''))
          const lines = doc.splitTextToSize(`•  ${clean}`, CW - 12)
          totalNoteLines += lines.length
          return lines
        })
        const specH = Math.max(16, 7.5 + totalNoteLines * 3.6 + 2)
        checkPage(specH + 3)

        setFill(GOLD_L); doc.roundedRect(ML, y, CW, specH, 1.5, 1.5, 'F')
        setDraw(GOLD); doc.setLineWidth(0.4); doc.roundedRect(ML, y, CW, specH, 1.5, 1.5, 'S')
        setFill(GOLD); doc.rect(ML, y, 2.5, specH, 'F')

        font('bold', 8); setTxt(NAVY)
        doc.text('SPECIAL INSTRUCTIONS & OPERATIONAL NOTES', ML + 5, y + 5.2)

        let sY = y + 9.2
        font('normal', 6.8); setTxt(NAVY)
        splitParagraphs.forEach(lines => {
          doc.text(lines, ML + 5, sY)
          sY += lines.length * 3.4 + 0.6
        })
        y += specH + 3
      }
      ...
      let notes = [
        ...
        'Rates subject to change due to peak seasons & public holidays.',
      ...
      const guestSlug = (landPackageOptions?.overrideGuestName || guestName || 'Guest').replace(/\s+/g, '-')
        ...
        for (let dayPos = 0; dayPos < endDayIdx - startDayIdx; dayPos++) {
          ...
          day.transfers.forEach(t => {
            ...
            if (desc.toLowerCase().startsWith('airport to hotel') || desc.toLowerCase().includes('arrival')) {
              ...
              const isSic = desc.toLowerCase().includes('sic') || isVehicleSIC(vObj)
              const ctText = isSic ? 'Half-Day City Tour (3 Hours) • Shared Coach (SIC)' : 'Half-Day City Tour (3 Hours) • Private 13-Seater Minibus'
        ...
        if (Array.isArray(rawItin) && rawItin.length > 0) {
          ...
          const sanitizedItin = rawItin.map((day: any) => ({
            ...
            attractions: Array.isArray(day.attractions) ? day.attractions.map((a: any) => {
              ...
              let resolvedPIdx = typeof a.pickupVehicleIndex === 'number' ? a.pickupVehicleIndex : undefined
              let resolvedDIdx = typeof a.dropVehicleIndex === 'number' ? a.dropVehicleIndex : undefined
              let resolvedPType = a.pickupVehicleType
              let resolvedDType = a.dropVehicleType
              if (vehiclesList.length > 0 && a.hasTransfer) {
                const pv = getAttractionVehicle(vehiclesList, a.pickupVehicleIndex, a.pickupVehicleType)
                const pFound = vehiclesList.indexOf(pv)
                if (pFound >= 0) {
                  resolvedPIdx = pFound
                  resolvedPType = pv.type
                }
                const dv = getAttractionVehicle(vehiclesList, a.dropVehicleIndex, a.dropVehicleType)
                const dFound = vehiclesList.indexOf(dv)
                if (dFound >= 0) {
                  resolvedDIdx = dFound
                  resolvedDType = dv.type
                }
              }
              return {
                ...a,
                pickupVehicleIndex: resolvedPIdx,
                pickupVehicleType: resolvedPType,
                dropVehicleIndex: resolvedDIdx,
                dropVehicleType: resolvedDType,
                attractionIndex: aIdx,
        ...
        const updatedAttractions = day.attractions.map(a => {
          ...
          if ((a.attractionIndex === -1 || a.attractionIndex === undefined) && a.attractionName) {
            ...
            if (matched) {
              ...
              if (f >= 0) {
                dayChanged = true
                attrUpdated = { ...attrUpdated, attractionIndex: f, adultPrice: a.adultPrice || matched.adultPrice, childPrice: a.childPrice || matched.childPrice }
              }
            }
          }
          if (vehiclesList.length > 0 && attrUpdated.hasTransfer) {
            const pv = getAttractionVehicle(vehiclesList, attrUpdated.pickupVehicleIndex, attrUpdated.pickupVehicleType)
            const pFound = vehiclesList.indexOf(pv)
            if (pFound >= 0 && (pFound !== attrUpdated.pickupVehicleIndex || pv.type !== attrUpdated.pickupVehicleType)) {
              dayChanged = true
              attrUpdated = { ...attrUpdated, pickupVehicleIndex: pFound, pickupVehicleType: pv.type }
            }
            const dv = getAttractionVehicle(vehiclesList, attrUpdated.dropVehicleIndex, attrUpdated.dropVehicleType)
            const dFound = vehiclesList.indexOf(dv)
            if (dFound >= 0 && (dFound !== attrUpdated.dropVehicleIndex || dv.type !== attrUpdated.dropVehicleType)) {
              dayChanged = true
              attrUpdated = { ...attrUpdated, dropVehicleIndex: dFound, dropVehicleType: dv.type }
            }
          }
          return attrUpdated
        })
...
${proposal}
    ...
    itinerary.forEach((day, dIdx) => {
      ...
      day.attractions.forEach(a => {
        ...
        if (scheduleFilters.transfers && a.hasTransfer) {
          if (a.pickupEnabled !== false) {
            const vehicle = getAttractionVehicle(vehiclesList, a.pickupVehicleIndex, a.pickupVehicleType)?.type || 'Vehicle'
          ...
          if (a.dropEnabled !== false) {
            const vehicle = getAttractionVehicle(vehiclesList, a.dropVehicleIndex, a.dropVehicleType)?.type || 'Vehicle'
  ...
  return (
    <div className="container" style={{ paddingTop: '1.5rem', paddingBottom: '4rem', maxWidth: '1600px', width: '96%' }}>
              ...
              .map((tmpl: any) => (
                ...
                >
                  ...
                  <div style={{ padding: '1rem 1.25rem 1.25rem', borderTop: '1px solid #F1F5F9', background: '#F8FAFC', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                    ...
                    >
                      <span>⚡</span> Quick Land Package Quote (No Hotels)
                    </button>
                    <button
                      type="button"
                      onClick={(e) => {
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                      ...
                      {(() => {
                        ...
                        day.attractions.forEach(a => {
                          ...
                          if (a.hasTransfer) {
                            if (a.pickupEnabled !== false) {
                              const pvName = getAttractionVehicle(vehiclesList, a.pickupVehicleIndex, a.pickupVehicleType)?.type || 'Vehicle'
                              items.push({
                                time: a.pickupTime || '09:00',
                                icon: '🚗',
                            ...
                            if (a.dropEnabled !== false) {
                              const dvName = getAttractionVehicle(vehiclesList, a.dropVehicleIndex, a.dropVehicleType)?.type || 'Vehicle'
                              items.push({
                                time: a.dropTime || '17:00',
                                icon: '🚗',
                                            ...
                                            {row.hasTransfer && destinationMode !== 'malaysia' && (
                                              <div style={{ marginTop: '0.5rem', background: '#F3E8FF', padding: '0.5rem 0.75rem', borderRadius: '6px', border: '1px solid #E9D5FF', display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                                                ...
                                                <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '0.5rem', fontSize: '0.75rem' }}>
                                                  ...
                                                  {(() => {
                                                    const pv = getAttractionVehicle(vehiclesList, row.pickupVehicleIndex, row.pickupVehicleType)
                                                    const pIdxVal = vehiclesList.indexOf(pv) >= 0 ? vehiclesList.indexOf(pv) : (row.pickupVehicleIndex ?? 0)
                                                    return (
                                                      <select
                                                        value={pIdxVal}
                                                        onChange={e => updateAttractionVehicle(dIdx, existingIdx, 'pickup', parseInt(e.target.value))}
                                                ...
                                                <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '0.5rem', fontSize: '0.75rem' }}>
                                                  ...
                                                  {(() => {
                                                    const dv = getAttractionVehicle(vehiclesList, row.dropVehicleIndex, row.dropVehicleType)
                                                    const dIdxVal = vehiclesList.indexOf(dv) >= 0 ? vehiclesList.indexOf(dv) : (row.dropVehicleIndex ?? 0)
                                                    return (
                                                      <select
                                                        value={dIdxVal}
                                                        onChange={e => updateAttractionVehicle(dIdx, existingIdx, 'drop', parseInt(e.target.value))}
                                                        style={{ padding: '0.2rem 0.35rem', borderRadius: '4px', border: '1px solid #CBD5E1', fontSize: '0.75rem', background: '#FFF', maxWidth: '200px' }}
                                                        disabled={row.dropEnabled === false}
                                                      >
                                                        {vehiclesList
      ...
      {/* ══════ READY-MADE LAND PACKAGE QUOTER MODAL (NO HOTELS) ══════ */}
      {landPackageModalItem && (
        <div 
          style={{ 
            position: 'fixed', 
            inset: 0, 
        ...
        >
          ...
          >
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '1rem 1.25rem', background: '#F8FAFC', display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', background: '#FFF', border: '1px solid #CBD5E1', borderRadius: '6px', height: '32px' }}>
                      <button 
                        type="button" 
                        onClick={() => {
                          const next = Math.max(0, landPackageKids - 1)
                        ...
                        {(day.transfers || []).length === 0 ? (
                          ...
                          (day.transfers || []).map((tr: any, tIdx: number) => {
                            ...
                            const trNet = tr.serviceType === 'arrival'
                              ? landPackageCalculation?.rate13Arrival || 45
                              : tr.serviceType === 'departure'
                              ? landPackageCalculation?.rate13Departure || 45
                              : tr.serviceType === 'disposal'
                              ? (landPackageCalculation?.rate13Disposal || 45) * (tr.hours || 4)
                              : tr.serviceType === 'cityTour'
                              ? (landPackageTransferMode === 'sic' ? (landPackageCalculation?.rateSicCityTour || 15) * Math.max(1, landPackageCalculation?.totalPax || 1) : landPackageCalculation?.rate13CityTour || 120)
              ...
              {landPackageAttractionModalDay !== null && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 1200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }} onClick={() => setLandPackageAttractionModalDay(null)}>
                  <div style={{ background: '#FFF', borderRadius: '12px', maxWidth: '520px', width: '100%', maxHeight: '80vh', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 20px 50px rgba(0,0,0,0.3)' }} onClick={e => e.stopPropagation()}>
                    <div style={{ padding: '1rem 1.25rem', background: '#0F4C3A', color: '#FFF', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{ fontWeight: 800, fontSize: '0.92rem' }}>
                        Select Attraction to Add (Day {landPackageAttractionModalDay + 1})
                      </span>
                      <button type="button" onClick={() => setLandPackageAttractionModalDay(null)} style={{ border: 'none', background: 'transparent', color: '#FFF', cursor: 'pointer', fontSize: '1rem' }}>✕</button>
                    </div>
                    <div style={{ padding: '0.75rem 1.25rem', borderBottom: '1px solid #E2E8F0' }}>
                      <input
                        type="text"
                        placeholder="Search attractions..."
                        value={landPackageSearchAttraction}
                        onChange={e => setLandPackageSearchAttraction(e.target.value)}
                      ...
                      {attractionsList
                        ...
                        .map((attr, aIdx) => {
                          ...
                          <div key={aIdx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.55rem 0', borderBottom: '1px solid #F1F5F9' }}>
                            ...
                            <button
                              type="button"
                              onClick={() => addLandPackageAttraction(landPackageAttractionModalDay, attr)}
