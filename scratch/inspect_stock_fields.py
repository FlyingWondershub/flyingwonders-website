...
r3 = requests.get(f'{BASE}/attraction/details?sku_id=RWSUSS', headers={'Authorization': f'BEARER {token}', 'X-API-Version': 'v1.10'})
tickets = r3.json().get('response', {}).get('data', {}).get('tickets', [])

for t in tickets:
    print("SKU:", t.get('sku_id'))
    print("Keys available:", list(t.keys()))
    print("stock / remaining / is_available fields:", {k: v for k, v in t.items() if 'stock' in k or 'rem' in k or 'avail' in k or 'status' in k or 'qty' in k or 'sold' in k})
    print("-" * 50)
