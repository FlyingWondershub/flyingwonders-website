...
r3 = requests.get(f'{BASE}/attraction/details?sku_id=RWSUSSGOACWSP', headers={'Authorization': f'BEARER {token}', 'X-API-Version': 'v1.10'})
print("RWSUSSGOACWSP details:", json.dumps(r3.json(), indent=2)[:1500])

r4 = requests.get(f'{BASE}/package/details?sku_id=RWSUSSGOACWSP', headers={'Authorization': f'BEARER {token}', 'X-API-Version': 'v1.10'})
print("package/details for RWSUSSGOACWSP:", json.dumps(r4.json(), indent=2)[:1500])
